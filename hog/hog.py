...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        ...
        if outcome == 1:
            ...
            sum += outcome
    test and print("The result is {}".format(sum))
    if is_pig_out:
        sum = 1
    return sum
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    score = 0
    if num_rolls == 0:
        digit0 = opponent_score % 10
        digit1 = opponent_score // 10
        score = max(digit0, digit1) + 1
    else: 
...
def bacon_strategy(score, opponent_score):
    ...
    if max(digit1,digit2) >= BACON_MARGIN - 1:
        return 0
    else: 
        return 5
...
def swap_strategy(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    roll_0_score = max(opponent_score%10, opponent_score//10) + 1
    if (score + roll_0_score) * 2 == opponent_score:
        return 0
    elif (score + roll_0_score) == 2 * opponent_score:
        return BASELINE_NUM_ROLLS
    elif roll_0_score >= BACON_MARGIN:
        return 0
    else: 
        return BASELINE_NUM_ROLLS
