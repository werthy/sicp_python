...
def analyze_tweet_sentiment(tweet):
    ...
    if total_num == 0:
        ...
        return make_sentiment(total_value/total_num)
...
def find_centroid(polygon):
    ...
    if area != 0:
        ...
        for index in range(0,n-1):
            centroid_latitude += (latitude(polygon[index])+latitude(polygon[index+1])) * (latitude(polygon[index])*longitude(polygon[index+1])-latitude(polygon[index+1])*longitude(polygon[index]))
            centroid_longitude += (longitude(polygon[index])+longitude(polygon[index+1])) * (latitude(polygon[index])*longitude(polygon[index+1])-latitude(polygon[index+1])*longitude(polygon[index]))
        centroid_latitude /= (6*area)
        centroid_longitude /= (6*area)
...
def find_state_center(polygons):
    ...
    for polygon in polygons:
        p1,p2,p3 = find_centroid(polygon)
        centroid_latitude += p1 * p3
        centroid_longitude += p2 * p3
        total_area += p3
    return make_position(centroid_latitude/total_area, centroid_longitude/total_area)
...
def group_tweets_by_state(tweets):
    ...
    "*** YOUR CODE HERE ***"
    state_centroid = {}
    for state_name, state in us_states.items():
        state_centroid[state_name] = find_state_center(state)
    for tweet in tweets:  # 这个算法 每找一个点都要遍历50个点..... 
        closest_state = ""
        closest_length = 9999.9
        tweet_loc = tweet_location(tweet)
        for state_name, state_center in state_centroid.items():
            dis = geo_distance(tweet_loc, state_center) # 这个地方呢，要用geo_distance（毕竟是球面嘛，不过怎么没人提醒一下捏
            if dis < closest_length:
                closest_length = dis
...
def average_sentiments(tweets_by_state):
    ...
    "*** YOUR CODE HERE ***"
    for state_name, tweets in tweets_by_state.items():
        # print(state_name, tweets)
        total_tweets_value = 0
        total_tweets_num = 0
        for tweet in tweets:
            sentiment = analyze_tweet_sentiment(tweet)
            if has_sentiment(sentiment):
                total_tweets_value += sentiment_value(sentiment)
                total_tweets_num += 1
        if total_tweets_num != 0:
            averaged_state_sentiments[state_name] = total_tweets_value/total_tweets_num
